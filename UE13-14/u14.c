...
{
  printf("ID: %d", song.id);
  printf("\nTitle: "); printTitle(song.id);
  printf("\nDuration: %02d:%02d", song.duration/60, song.duration%60);
  printf("\nPlayed today: %d\n\n", song.play_count);
}
...
{
  int played = songs[position - 1].play_count;
  //printf("%d\n", played);
  printTitle(position);
  if (played >= 5) {
...
{
  positionSong1--; positionSong2--;
  Song temp = songs[positionSong1];
  songs[positionSong1] = songs[positionSong2];
  songs[positionSong2] = temp;
}
